...
public class Enemy : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        var enemyToPlayer = nearestPlayer.transform.position - transform.position;
        enemyToPlayer.z = 0.0f;
        enemyToPlayer.Normalize();
        float angle = Vector3.Angle(enemyToPlayer, Vector3.up) * -math.sign(enemyToPlayer.x);

        mRigidBody.MovePosition(transform.position + (nearestPlayer.transform.position - transform.position) * speed * Time.fixedDeltaTime);
